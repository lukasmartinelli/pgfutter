...
function query_counts() {
    local table="$1"
    local counts=$(psql -h "${DB_HOST}" -U "${DB_USER}" -d "${DB_NAME}" -t -c "select count(*) from ${DB_SCHEMA}.${table}")
    echo "$counts"
}

function query_field_type() {
    local table="$1"
    local data_type=$(psql -h "${DB_HOST}" -U "${DB_USER}" -d "${DB_NAME}" -t -c "SELECT data_type FROM information_schema.columns WHERE table_schema='${DB_SCHEMA}' AND table_name='${table}'")
    echo "$data_type"
}
...
function import_csv_with_special_delimiter_and_trailing() {
    ...
    pgfutter --schema "$DB_SCHEMA" --db "$DB_NAME" --user "$DB_USER" --pass "$PGPASSWORD" csv --delimiter=";" "$filename"
    if [ $? -ne 0 ]; then
...
function test_excel_csv() {
    ...
    pgfutter $general_args --table "$table" --schema "$DB_SCHEMA" --db "$DB_NAME" --user "$DB_USER" --pass "$PGPASSWORD" csv --delimiter "$delimiter" --excel "$filename"
    if [ $? -ne 0 ]; then
        echo "pgfutter could not import $filename"
        exit 300
    else
...
function test_csv() {
    ...
    pgfutter $general_args --table "$table" --schema "$DB_SCHEMA" --db "$DB_NAME" --user "$DB_USER" --pass "$PGPASSWORD" csv --delimiter "$delimiter" "$filename"
    if [ $? -ne 0 ]; then
        echo "pgfutter could not import $filename"
        exit 300
    else
